...
document.getElementById('guid').value=profileId;
...
if (verificationProfile.remainingEnrollments == 0)
{
window.alert("created verificationprofile successfully, now click 'login' button");
}
};
...
verificationProfile =(new VerificationProfile(document.getElementById("username").value, document.getElementById("guid").innerHTML));
...
if(a.result=="Accept")
{
  ...
  window.alert("voice is not recognized, try logging in again");
}
};
...
onMediaSuccess(stream, createProfile, 15);}, onMediaError);
}
...
//var p =JSON.parse(document.getElementById('j').innerHTML);
profileIds.push(new Profile(document.getElementById("username").value, document.getElementById("guid").innerHTML));
...
//var Ids = "244b4131-26b7-4079-90b5-c7a409a09e3c";
...
document.getElementById('id').value=json.processingResult.identificationProfileId;
if (speaker != null && speaker.length > 0){
window.alert("identified successfully, attendance is updated");
} else {
...
var key = "c292288a99b749dd9713434e1b3b12d4";
